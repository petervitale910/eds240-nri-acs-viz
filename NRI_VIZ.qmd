...
ggplot(aes(x = state_name, y = national_risk_index_score_composite))+
  ...
  gghighlight(state_name == 'California', # Highlight california
              unhighlighted_params = list(colour = NULL, alpha = 0.7))+ #Set the alpha and color of the unhighlighted
  coord_flip()+
  ggthemes::theme_wsj() # Wallstreet Journal Theme

# Repeat the code but change the selected region
...
ggplot( aes(x = state_name, y = national_risk_index_score_composite))+
  ...
  gghighlight(state_name == 'California',
              unhighlighted_params = list(colour = NULL, alpha = 0.7))+
  coord_flip()+
  ggthemes::theme_wsj()
...
-  The theme_wsj makes the plot more intelligable, furthermore my use of a larger title and subtitle make the visualization easy to understand and interpret. 
